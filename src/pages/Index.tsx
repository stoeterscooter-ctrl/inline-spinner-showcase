...
const Index = () => {
  ...
  return (
    <ComponentShowcase
      ...
      useCases={
        <>
          <Button variant="secondary" size="sm" className="h-8 w-24 gap-1.5 text-xs" onClick={save.trigger} disabled={save.state !== "idle"}>
          ...
          <Button size="sm" className="h-8 w-24 gap-1.5 text-xs" onClick={post.trigger} disabled={post.state !== "idle"}>
          ...
          <Button size="sm" className="h-8 w-24 gap-1.5 text-xs" onClick={publish.trigger} disabled={publish.state !== "idle"}>
            {publish.state === "idle" && "Publish"}
            {publish.state === "loading" && <Spinner variant="blade" size="xs" />}
            {publish.state === "done" && <><Check className="h-3 w-3" />Done</>}
          </Button>

          <div className="w-px h-5 bg-border mx-1" />

          <Button variant="outline" size="sm" className="h-8 gap-1.5 text-xs" onClick={google.trigger} disabled={google.state !== "idle"}>
          ...
          <Button variant="outline" size="sm" className="h-8 gap-1.5 text-xs" onClick={github.trigger} disabled={github.state !== "idle"}>
